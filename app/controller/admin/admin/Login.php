...
class Login extends Common
{
    ...
    public function index()
    {
        if ($this->request->isAjax()) {
            ...
            $items   = $this->service->getLoginLogList($filter, [], $orderby, $page, $limit);
            $count   = $this->service->getLoginLogCount($filter);
            $filter  = [
            ...
            foreach ($items as &$item) {
                ...
                if (!isset($roles[$users[$item['user_id']]['role_id']]['root'])) {
                    $item['role_root'] = '角色已删除';
                } else {
